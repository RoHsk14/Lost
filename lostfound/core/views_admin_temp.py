...
def admin_users(request):
    ...
    # Admin peut maintenant voir TOUS les utilisateurs sans restriction
    if user.role == 'admin':
        # Admin a maintenant un accès total à tous les utilisateurs
        utilisateurs = Utilisateur.objects.select_related('region', 'prefecture')
    else:
    ...
    if user.role == 'admin':
        # Admin voit les statistiques de tous les citoyens et agents
        base_query = base_query.filter(role__in=['citoyen', 'agent'])
    # Admin a maintenant un accès total à tous les utilisateurs
    if user.role == 'admin':
    ...
    if user.role == 'superadmin':
        stats_utilisateurs['admins'] = {
            'total': Utilisateur.objects.filter(role='admin').count(),
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                log_action(
                    ...
                    description=f"Administrateur {updated_admin.username} modifié par {request.user.username}",
                    donnees_supplementaires={'admin_id': updated_admin.id}
                )
                
                messages.success(request, f"Administrateur {updated_admin.username} modifié avec succès")
                return redirect('superadmin:gestion_admins')
        else:
...
def delete_admin(request, admin_id):
    ...
    try:
        ...
        context = {'admin': admin}
        return render(request, 'superadmin/admin_delete_confirm.html', context)
...
def signalements_list(request):
    ...
    region_filter = Q()
    if user.region and user.role not in ['superadmin']:
...
def signalement_detail(request, signalement_id):
    ...
    if user.region and signalement.region != user.region and user.role != 'superadmin':
    ...
    context = {
        ...
        'commentaires': commentaires,
        'can_edit': True,
        'can_delete': user.role in ['admin', 'superadmin'],
    }
...
def signalement_delete(request, signalement_id):
    """Supprimer un signalement (admin et superadmin uniquement)"""
    user = request.user
